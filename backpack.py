...
class BpxClient:
    ...
    def sendOrder(self, symbol, side, orderType, timeInForce, quantity, price):
        params = {
            'symbol': symbol,
            'side': side,
            'orderType': orderType,
    ...
    # Trades
    def recentTrades(self, symbol: str, limit: int = 100):
        return requests.get(url=f'{self.url}api/v1/trades?symbol={symbol}&limit={limit}').json()

    def historyTrades(self, symbol: str, limit: int = 100, offset: int = 0):
        return requests.get(url=f'{self.url}api/v1/trades/history?symbol={symbol}&limit={limit}&offset={offset}').json()